...
public final class TieredStorageBridge {
    ...
    static {
        ...
        // i64 ts_create_tiered_object_store(i64 local, i64 remote, i64 cache, i64 resolve_cb, i32 skip_registry)
        CREATE = linker.downcallHandle(
            ...
            FunctionDescriptor.of(
                ...
                ValueLayout.JAVA_INT     // skip_registry_lookup (0 = normal, 1 = skip for testing)
            )
        );
    ...
    public static long createTieredObjectStore(long localStorePtr, long remoteStorePtr, long cacheBoxPtr) {
        try {
            return NativeLibraryLoader.checkResult((long) CREATE.invokeExact(localStorePtr, remoteStorePtr, cacheBoxPtr, 0L, 0));
        } catch (Throwable t) {
    ...
    public static long createTieredObjectStoreWithResolver(
        ...
        return createTieredObjectStoreWithResolver(localStorePtr, remoteStorePtr, cacheBoxPtr, resolveStub, false);
    }

    /**
     * Create a TieredObjectStore with all options including skip_registry_lookup for testing.
     *
     * @param localStorePtr      Box&lt;Arc&lt;dyn ObjectStore&gt;&gt; pointer, or 0 for default LocalFileSystem
     * @param remoteStorePtr     Box&lt;Arc&lt;dyn ObjectStore&gt;&gt; pointer, or 0 for no remote
     * @param cacheBoxPtr        block cache pointer, or 0 for no cache
     * @param resolveStub        upcall stub for lazy file resolution
     * @param skipRegistryLookup if true, skip the registry check (forces all reads through lazy resolver)
     * @return native Arc&lt;TieredObjectStore&gt; pointer
     */
    public static long createTieredObjectStoreWithResolver(
        long localStorePtr,
        ...
        long cacheBoxPtr,
        java.lang.foreign.MemorySegment resolveStub,
        boolean skipRegistryLookup
    ) {
        try {
            long resolveAddr = (resolveStub != null) ? resolveStub.address() : 0L;
            int skip = skipRegistryLookup ? 1 : 0;
            return NativeLibraryLoader.checkResult(
                (long) CREATE.invokeExact(localStorePtr, remoteStorePtr, cacheBoxPtr, resolveAddr, skip)
            );
        } catch (Throwable t) {
