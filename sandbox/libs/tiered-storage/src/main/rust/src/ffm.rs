...
///   Signature: `(path_ptr: *const u8, path_len: i64) -> i64`. Returns 1 if the file was
///   resolved and registered, 0 otherwise. If 0, no lazy resolution.
/// - `skip_registry_lookup`: if non-zero (1), skips the initial registry check and forces
///   all reads through the lazy resolver. Use for testing the upcall path end-to-end.
#[ffm_safe]
#[no_mangle]
pub extern "C" fn ts_create_tiered_object_store(
    ...
    if skip_registry_lookup != 0 {
        store.skip_registry_lookup = true;
        native_bridge_common::log_info!("ffm: ts_create_tiered_object_store: skip_registry_lookup=true (test mode)");
    }
    ...
    native_bridge_common::log_info!(
        "ffm: ts_create_tiered_object_store cache_wired={} resolve_cb_wired={} skip_registry={}",
        cache_box_ptr != NULL_PTR,
        resolve_cb_ptr != NULL_PTR,
        skip_registry_lookup != 0
    );
    Ok(ptr)
}
