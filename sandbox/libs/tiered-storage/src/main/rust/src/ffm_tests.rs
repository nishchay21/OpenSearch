...
fn test_create_and_destroy_no_leak() {
    let store_ptr = ts_create_tiered_object_store(0, 0, 0, 0, 0);
    assert!(store_ptr > 0);
...
fn test_create_with_remote_does_not_consume_pointer() {
    ...
    let store1 = ts_create_tiered_object_store(0, remote_ptr, 0, 0, 0);
    let store2 = ts_create_tiered_object_store(0, remote_ptr, 0, 0, 0);
    assert!(store1 > 0);
    assert!(store2 > 0);
...
fn test_create_with_cache_does_not_consume_pointer() {
    ...
    let store1 = ts_create_tiered_object_store(0, 0, cache_ptr, 0, 0);
    let store2 = ts_create_tiered_object_store(0, 0, cache_ptr, 0, 0);
    assert!(store1 > 0);
    assert!(store2 > 0);
