...
public interface TieredStoreNativeBridge {
     ...
     *
     * @param diskCacheBytes total disk budget for the Foyer cache in bytes; {@code 0} disables caching
     * @param diskCacheDir   local NVMe directory for Foyer data files (ignored when {@code diskCacheBytes == 0})
     * @return {@code long[3]}: {@code [objectStoreDataPtr, objectStoreVtablePtr, registryPtr]}
     */
    long[] createTieredObjectStore(long diskCacheBytes, String diskCacheDir);
