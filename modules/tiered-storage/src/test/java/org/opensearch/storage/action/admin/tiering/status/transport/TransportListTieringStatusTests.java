...
public class TransportListTieringStatusTests extends OpenSearchTestCase {
    ...
    public void testClusterManagerOperation_WithTargetTierWarm() throws IOException {
        ...
        List<TieringStatus> expectedStatus = Arrays.asList(
                createDummyTieringStatus("index1"),
                createDummyTieringStatus("index2")
        );
        ...
        // Assert
        verify(listener).onResponse(argThat(response ->
                response.getTieringStatusList().size() == 2 &&
                        response.getTieringStatusList().equals(expectedStatus)
        ));
    }
    ...
    public void testClusterManagerOperation_WithoutTargetTier() throws IOException {
        ...
        List<TieringStatus> hotToWarmStatus = Arrays.asList(
                createDummyTieringStatus("index1")
        );
        List<TieringStatus> warmToHotStatus = Arrays.asList(
                createDummyTieringStatus("index2")
        );
