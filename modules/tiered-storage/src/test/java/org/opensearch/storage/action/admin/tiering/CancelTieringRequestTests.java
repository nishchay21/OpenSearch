...
public class CancelTieringRequestTests extends OpenSearchTestCase {
    ...
    public void testValidationWithNullIndex() {
        ...
        assertThat("Validation message should be appropriate", 
            validation.getMessage(), containsString("Index name cannot be null or empty"));
    }
    ...
    public void testSerializationRoundTrip() throws IOException {
        ...
        assertEquals("Index should be preserved after serialization", 
            original.getIndex(), deserialized.getIndex());
        assertEquals("Timeout should be preserved after serialization",
            original.timeout(), deserialized.timeout());
        assertEquals("Cluster manager timeout should be preserved after serialization",
            original.clusterManagerNodeTimeout(), deserialized.clusterManagerNodeTimeout());
    }
    ...
    public void testEqualityWithOneNullIndex() {
        ...
        assertFalse("Request with null index should not equal request with non-null index", 
            request1.equals(request2));
        assertFalse("Request with non-null index should not equal request with null index", 
            request2.equals(request1));
    }
    ...
    public void testEqualityWithDifferentClass() {
        ...
        assertFalse("Request should not be equal to object of different class", 
            request.equals(otherObject));
    }
    ...
    public void testToString() {
        ...
        assertTrue("toString should contain class name", 
            toString.contains("CancelTieringRequest"));
        assertTrue("toString should contain index name", 
            toString.contains(indexName));
    }
    ...
    public void testClusterManagerNodeTimeoutFunctionality() {
        ...
        assertNotNull("Default cluster manager timeout should not be null", 
        ...
        assertEquals("Cluster manager timeout value should match", "1.5m",
            request.clusterManagerNodeTimeout().toString());
    }
    ...
    public void testMultipleValidIndexNames() {
        ...
        for (String indexName : validIndices) {
            CancelTieringRequest request = new CancelTieringRequest();
            request.setIndex(indexName);
            assertNull("Index '" + indexName + "' should pass validation", 
                request.validate());
        }
    }
}