...
public class TieringStatusIT extends RemoteStoreBaseIntegTestCase {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins() {
        return Stream.concat(super.nodePlugins().stream(), Stream.of(TieredStoragePlugin.class, MockInternalClusterInfoService.TestPlugin.class))
    ...
    protected void assertIndexShardLocation(String indexName, boolean onWarmNodes) {
        ...
        for (IndexShardRoutingTable shardRoutingTable : indexRoutingTable) {
            ...
            if (onWarmNodes) {
                assertTrue("Shard should be on warm node",
                        assignedNode.getRoles().contains(DiscoveryNodeRole.WARM_ROLE));
            } else {
                assertFalse("Shard should not be on warm node",
                        assignedNode.getRoles().contains(DiscoveryNodeRole.WARM_ROLE));
            }
        }
    }
    ...
    protected void initiateStuckMigrationToGetStatus(Map<String, String> indexToTargetTier, Map<String, Integer> expectedListTieringResponseCount) throws Exception {

        logger.info("Updating cluster setting to pause recoveries");
        client().admin().cluster().prepareUpdateSettings()
                .setTransientSettings(buildDisabledAllocationSettings()).get();
        ...
        try {
            ...
            client().admin().cluster().prepareUpdateSettings()
                    .setTransientSettings(buildEnabledAllocationSettings(2)).get();
        }
    ...
    protected void verifyTieringStatusResponse(ListTieringStatusRequest request, int expectedCount) {
        ListTieringStatusResponse response = client().admin().indices()
                .execute(ListTieringStatusAction.INSTANCE, request)
                .actionGet();
    ...
    protected void verifyIndividualTieringStatus(Map<String, String> indexToTargetTier) {
        for (Map.Entry<String, String> entry : indexToTargetTier.entrySet()) {
            ...
            GetTieringStatusResponse tieringStatusResponse = client().admin().indices()
            ...
            assertEquals(0, tieringStatusResponse.getTieringStatus().getShardLevelStatus().getShardLevelCounters().get(RUNNING_SHARDS).intValue());
        }
    }
