...
public class TierCancelIT extends RemoteStoreBaseIntegTestCase {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins() {
        return Stream.concat(
                ...
                // Add tiered storage plugin to the list
    ...
    public void testCancelHotToWarmMigrationSuccess() throws Exception {
        ...
        try {
            ...
            Thread.sleep(2000); // Let migration attempt start
            
            // Verify migration is in progress but stuck
            assertBusy(() -> {
                ClusterState state = client().admin().cluster().prepareState().get().getState();
                Settings indexSettings = state.metadata().index(INDEX_NAME).getSettings();
                assertEquals("Index should have warm setting",
                    "true", indexSettings.get(IS_WARM_INDEX_SETTING.getKey()));
                assertEquals("Index should be in HOT_TO_WARM state",
            ...
            refresh(INDEX_NAME);
            assertEquals("Document count should be preserved",
    ...
    public void testCancelWarmToHotMigrationSuccess() throws Exception {
        ...
        try {
            ...
            assertBusy(() -> {
                ClusterState state = client().admin().cluster().prepareState().get().getState();
                Settings indexSettings = state.metadata().index(INDEX_NAME).getSettings();
                assertFalse("Index should have warm setting",
                        IS_WARM_INDEX_SETTING.get(indexSettings));
                assertEquals("Index should be in WARM_TO_HOT state",
    ...
    public void testCancelIndexNotBeingTiered() {
        ...
        try {
            ...
            verifyCancellationError(INDEX_NAME, 
                "Index [" + INDEX_NAME + "] is not currently undergoing tiering operation");
        } finally {
            client().admin().indices().delete(new DeleteIndexRequest(INDEX_NAME)).actionGet();
        }
    }
    ...
    public void testCancelWithTimeoutParameters() throws Exception {
        ...
        try {
            ...
            AcknowledgedResponse response = client().admin().indices()
                .execute(CancelTieringAction.INSTANCE, request)
                .actionGet();
    ...
    protected void verifyIndexInHotTier(String indexName) throws Exception {
        assertBusy(() -> {
            ...
            assertFalse("index.warm setting should be removed",
                    IS_WARM_INDEX_SETTING.get(indexSettings));
