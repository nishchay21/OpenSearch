...
public class WarmToHotTieringServiceTests extends OpenSearchTestCase {
    ...
    public void testGetTieringMaxConcurrentRequests() {
        assertEquals(W2H_MAX_CONCURRENT_TIEIRNG_REQUESTS_KEY, service.getMaxConcurrentTieringRequestsSetting().getKey());
    }
    ...
    public void testIsShardInTargetTier() {
        ...
        when(node.isWarmNode()).thenReturn(false);  // Note: false for HOT tier
    ...
    public void testClusterChanged_WithRoutingTableChange_AndExistingIndex() {
        ...
        when(clusterState.getNodes()).thenReturn(nodes);
        when(nodes.get("node1")).thenReturn(node);
        when(node.isWarmNode()).thenReturn(false); // Should be on hot node
    ...
    public void testClusterChanged_WithoutRoutingTableChange() {
        ClusterChangedEvent event = mock(ClusterChangedEvent.class);
        ClusterState previousState = mock(ClusterState.class);
        DiscoveryNodes previousNodes = mock(DiscoveryNodes.class);
        ...
        // Setup routing table change check
    ...
    public void testClusterChanged_NewMasterWithInProgressRequests() {
        ClusterChangedEvent event = mock(ClusterChangedEvent.class);
        ClusterState previousState = mock(ClusterState.class);
        DiscoveryNodes previousNodes = mock(DiscoveryNodes.class);
        Metadata metadata = mock(Metadata.class);
    ...
    public void testProcessTieringInProgress_WithUnassignedReplica() {
        ...
        List<ShardRouting> shardRoutings = Arrays.asList(
            createShardRouting(true, false, "node1", true),  // primary shard
            createShardRouting(false, true, null, false)     // unassigned replica
        );

        // Setup master node context
        setupMasterNodeContext(event);

        // Setup routing table and nodes
        when(event.routingTableChanged()).thenReturn(true);
        when(event.state()).thenReturn(clusterState);
        when(clusterState.routingTable()).thenReturn(routingTable);
        when(routingTable.hasIndex(testIndex)).thenReturn(true);
        ...
        when(clusterState.getNodes()).thenReturn(nodes);
        when(nodes.get("node1")).thenReturn(node);
        when(node.isWarmNode()).thenReturn(false); // Should be on hot node for warm-to-hot migration
        ...
        // Verify that unassigned replica doesn't prevent migration completion
        verify(routingTable).allShards(testIndex.getName());
        verify(nodes).get("node1");
        verify(node).isWarmNode();
    }

    private ShardRouting createShardRouting(boolean primary, boolean unassigned,
                                            String nodeId, boolean started) {
        ShardRouting shard = mock(ShardRouting.class);
    ...
    private void setupMasterNodeContext(ClusterChangedEvent event) {
        ClusterState previousState = mock(ClusterState.class);
        DiscoveryNodes previousNodes = mock(DiscoveryNodes.class);
        when(event.localNodeClusterManager()).thenReturn(true);
        when(event.previousState()).thenReturn(previousState);
    ...
    private void setupRoutingTable(ClusterChangedEvent event, RoutingTable routingTable,
