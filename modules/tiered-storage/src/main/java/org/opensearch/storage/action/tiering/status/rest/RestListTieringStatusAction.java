...
public class RestListTieringStatusAction extends AbstractCatAction {
    ...
    @Override
    public RestChannelConsumer doCatRequest(RestRequest restRequest, NodeClient nodeClient) {
        String targetTier = restRequest.param(TARGET_TIER);
        ...
        return channel ->
                nodeClient.admin().cluster().execute(ListTieringStatusAction.INSTANCE, listTieringStatusRequest, new RestResponseListener<ListTieringStatusResponse>(channel) {
                    @Override
                    public RestResponse buildResponse(ListTieringStatusResponse listTieringStatusResponse) throws Exception {
                        return RestTable.buildResponse(buildTable(restRequest, listTieringStatusResponse), channel);
                    }
                });
    }
