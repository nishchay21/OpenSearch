...
public abstract class TieringService implements ClusterStateListener {
    ...
    /**
     * Service for managing cluster operations and state updates.
     */
    protected final ClusterService clusterService;
    /**
     * Service that provides information about cluster resources and statistics.
     */
    protected final ClusterInfoService clusterInfoService;
    /**
     * Resolver for handling index name expressions and aliases.
     */
    protected final IndexNameExpressionResolver indexNameExpressionResolver;
    /**
     * Service for managing shard allocation in the cluster.
     */
    protected final AllocationService allocationService;
    /**
     * Context of set of indices currently undergoing tiering operations.
     */
    protected final Set<Index> tieringIndices;
    /**
     * Settings that control disk usage thresholds and related behaviors.
     */
    protected final DiskThresholdSettings diskThresholdSettings;
    /**
     * Settings that control the behavior of the file cache system.
     */
    protected final FileCacheSettings fileCacheSettings;
    /**
     * Shard Limit Validator.
     */
    ...
    private Integer jvmActiveUsageThresholdPercent;
     /**
     * Metrics registry for tracking tiering operations success, rejections, and latency.
     */
    protected final TierActionMetrics tierActionMetrics;
    /**
     * Current node identifier used for metrics tagging.
     */
    ...
    protected TieringService(
        ...
        this.tierActionMetrics = tierActionMetrics;
    ...
    /**
     * Returns the settings to be added when tiering starts
     */
    protected abstract Settings getTieringStartSettingsToAdd();

    /**
     * Returns the original tier settings to be restored back when tiering is cancelled midway
    */
    protected abstract Settings getIndexTierSettingsToRestoreAfterCancellation();

    /**
     * Returns the key used to store the tiering start time in metadata
     */
    protected abstract String getTieringStartTimeKey();

    /**
     * Returns the setting used to store the maximum tiering queue size
     * */
    protected abstract Setting<Integer> getMaxConcurrentTieringRequestsSetting();

    /**
     * Returns the target tiering state (HOT/WARM) for this tiering
     */
    protected abstract IndexModule.TieringState getTargetTieringState();

    /**
     * Performs validation checks for the tiering request against current cluster state
     */
    protected abstract void validateTieringRequest(final ClusterState clusterState,
     ...
     */
    protected void setMaxConcurrentTieringRequests(Integer maxConcurrentTieringRequests) {
     ...
     */
    protected void setJvmActiveUsageThresholdPercent(Integer jvmActiveUsageThresholdPercent) {
    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        if (event.localNodeClusterManager()) {
            if (event.previousState().nodes().isLocalNodeElectedClusterManager() == false || (event.blocksChanged() && event.previousState().blocks().hasGlobalBlock(STATE_NOT_RECOVERED_BLOCK) && !event.state().blocks().hasGlobalBlock(STATE_NOT_RECOVERED_BLOCK))) {
                // Master node got changed, reconstruct the inProgressIndices.
            ...
            if (event.routingTableChanged() && tieringIndices.isEmpty() == false) {
                logger.debug(() -> String.format(Locale.ROOT, "[%s] processing %d in-progress requests after routing table update", source, tieringIndices.size()));
                processTieringInProgress(event.state(), getTieringSourceType(getTargetTieringState()));
            }
        }
    }

    void reconstructInProgressTieringRequests(final ClusterState clusterState, final IndexModule.TieringState tieringState, final String source) {
                ...
                .forEach(indexMetadata -> {
                    try {
                        ...
                        if (tieringState.toString().equals(indexTieringState)) {
                            ...
                            logger.info(() -> String.format(Locale.ROOT, "[%s] added index [%s] to in-progress migration tracking",
                                    source, index.getName()));
                        }
                    } catch (Exception e) {
                        logger.error("Failed to process index [{}] during reconstruction. Error: {}",
                                indexMetadata.getIndex().getName(), e);
                    }
                });
    }
    ...
    void processTieringInProgress(final ClusterState clusterState, final String source) {
        // Create a batch of completed indices
        ...
        for (Index index : tieringIndices) {
            ...
            if (allShardRelocationComplete) {
                logger.info(() -> String.format(Locale.ROOT, "Shard relocation for [%s] completed successfully for index [%s]", source, index.getName()));
    ...
    private void updateClusterStateForTieredIndices(final Set<Index> completedIndices, final String source) {
        ...
        clusterService.submitStateUpdateTask("complete " + source + " for indices: " + completedIndices, new ClusterStateUpdateTask(Priority.NORMAL) {
            @Override
                ...
                for (Index index : completedIndices) {
                    if (tieringIndices.contains(index)) {
                        ...
                        if (indexMetadata == null) {
                            logger.info(() -> String.format(Locale.ROOT, "Index [%s] not found while completing %s", index.getName(), source));
                            tieringIndices.remove(index);
                            continue;
                        }
            ...
            public void onFailure(String source, Exception e) {
                logger.error(
                    (Supplier<?>) () -> new ParameterizedMessage(
                        "Failed to complete task {} for indices " + "[{}]",
                        source,
                        completedIndices
                    ),
                    e
            ...
            public void clusterStateProcessed(String source, ClusterState oldState, ClusterState newState) {
                ...
                for (Index index : completedIndices) {
                    if (tieringIndices.contains(index)) {
                        tierActionMetrics.recordSuccessfulMigration(
                            nodeId,
                            index.getName(),
                            getTieringType().toString()
                        );

                        long startTime = TieringUtils.getTieringStartTime(oldState, index, getTieringStartTimeKey());
                        long endTime = System.currentTimeMillis();
                        double latencyMs = (double) (endTime - startTime);

                        tierActionMetrics.recordMigrationLatency(
                            latencyMs,
                            nodeId,
                            index.getName(),
                            getTieringType().toString()
     ...
     *
     * @param request the cancel tiering request containing index information
     * @param listener callback listener for the cancellation operation
     * @param state current cluster state
     */
    public void cancelTiering(final CancelTieringRequest request, final ActionListener<ClusterStateUpdateResponse> listener, final ClusterState state) {
        try {
            ...
            clusterService.submitStateUpdateTask(source, new ClusterStateUpdateTask(Priority.IMMEDIATE) {
                ...
                public void onFailure(String source, Exception e) {
                    logger.error(() -> new ParameterizedMessage(
                        "Failed to cancel tiering for index [{}]",
                        request.getIndex()
                    ), e);
                ...
                @Override
                public void clusterStateProcessed(String source, ClusterState oldState, ClusterState newState) {
                    logger.info(() -> String.format(Locale.ROOT, "Successfully cancelled [%s] tiering for index [%s]", getTieringSourceType(getTargetTieringState()), index.getName()));
                    tieringIndices.remove(index);
    ...
    /**
     * Checks if an index is currently being tiered by this service.
     *
     * @param index tiering cancel request index
     * @return true if the index is being tiered, false otherwise
     */
     ...
     * @param state current cluster state
     */
    public void tier(final IndexTieringRequest request, final ActionListener<ClusterStateUpdateResponse> listener, final ClusterState state) {
        try {
            final Index index = resolveRequestIndex(indexNameExpressionResolver, request.getIndex(), state);
                ...
                public ClusterState execute(ClusterState currentState) {
                    ...
                    try {
                        validateTieringRequest(currentState, clusterInfoService, tieringIndices,
                                maxConcurrentTieringRequests, jvmActiveUsageThresholdPercent, index);
                    } catch (TieringRejectionException e) {
                        tierActionMetrics.recordRejectedMigration(
                            nodeId,
                            index.getName(),
                            getTieringType().toString(),
                            e.getRejectionReason().name()
                        );
    ...
    void updateIndexMetadataForTieringStart(
        ...
        try {
            ...
            Settings.Builder indexSettingsBuilder = Settings.builder()
                ...
                .put(getTieringStartSettingsToAdd());
            ...
            throw new OpenSearchException("Failed to update index metadata for tiering start", e.getCause());
        }
    }
     ...
     * @param indexMetadata current metadata of the index
     */
    void updateIndexMetadataPostTiering(
        final Metadata.Builder metadataBuilder,
        final IndexMetadata indexMetadata
    ) {
        try {
            ...
            throw new OpenSearchException("Failed to update index metadata post tiering", e.getCause());
        }
    }
     ...
     * @param indexMetadata current metadata of the index
     */
    void updateIndexMetadataForTieringCancel(
        final Metadata.Builder metadataBuilder,
        final IndexMetadata indexMetadata
    ) {
        try {
            ...
            throw new OpenSearchException("Failed to update index metadata for tiering cancellation", e.getCause());
        }
    }
     ...
     */
    void ValidateTieringCancelRequest(
        final Index index,
        final IndexMetadata indexMetadata,
        ...
        if (indexSettings.get(INDEX_TIERING_STATE.getKey(), "") == getTargetTieringState().toString()) {
            throw new IllegalArgumentException("Index [" + index + "] already reached its target tier: " + getTargetTieringState().toString() + ". Cannot cancel tiering");
        }
    ...
    private TieringStatus constructTieringStatus(Index index, boolean shardLevelStatus, boolean isDetailedFlagEnabled) {
        ...
        String source_tier = tiers[0];
        String destination_tier = tiers[1];
        tieringStatus = new TieringStatus(index.getName(), TIERING_IN_PROGRESS_STATUS,
                source_tier, destination_tier, tieringStartTime);

        if (shardLevelStatus) {
            tieringStatus.setShardLevelStatus(TieringStatus.ShardLevelStatus.fromRoutingTable(clusterService.state(), index.getName(), isDetailedFlagEnabled, destination_tier));
        }
