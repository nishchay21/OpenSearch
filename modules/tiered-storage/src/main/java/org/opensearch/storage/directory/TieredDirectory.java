...
import static org.opensearch.index.store.Store.CORRUPTED_MARKER_NAME_PREFIX;
...
public class TieredDirectory extends CompositeDirectory {
    ...
    public TieredDirectory(Directory localDirectory, Directory remoteDirectory, FileCache fileCache, ThreadPool threadPool, Supplier<TieredStoragePrefetchSettings> tieredStoragePrefetchSettingsSupplier) {
    ...
    public String[] listAll() throws IOException {
        ...
        boolean hasLocalSegments = Arrays.stream(localFiles)
            .anyMatch(fileName -> fileName.startsWith(IndexFileNames.SEGMENTS));
    ...
    public IndexInput openInput(String name, IOContext context) throws IOException {
        ...
        if (FileTypeUtils.isTempFile(name) || isCorruptedFile(name)) {
        ...
        if (indexInput == null) {
            ...
            // For pre-created files already in remote, switchable entry would not be cached already locally, we need to download from remote and cache it in FileCache
            cacheFile(name, true);
            indexInput = fileCache.get(key);
            // decrementing the ref here since refCount increases in cacheFile method, but since afterSyncToRemote won't be called (where we decRef) for files already in remote
    ...
    protected void cacheFile(String fileName, boolean cacheFromRemote) throws IOException {
        ...
        fileCache.put(
            ...
            new CachedSwitchableIndexInput(
                fileCache, fileName, localDirectory, remoteDirectory, transferManager, cacheFromRemote, threadPool, tieredStoragePrefetchSettingsSupplier
            )
        );
    }

    private boolean isCorruptedFile(final String name) {
        return name.startsWith(CORRUPTED_MARKER_NAME_PREFIX);
    }
}
