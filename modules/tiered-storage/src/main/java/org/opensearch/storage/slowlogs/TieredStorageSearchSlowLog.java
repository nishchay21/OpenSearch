...
public final class TieredStorageSearchSlowLog implements SearchOperationListener {
    ...
    private boolean tieredStorageSlowlogEnabled;
    ...
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_WARN_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.query.warn", TimeValue.timeValueMillis(10000),
    ...
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_INFO_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.query.info", TimeValue.timeValueMillis(5000),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_DEBUG_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.query.debug", TimeValue.timeValueMillis(2000),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_TRACE_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.query.trace", TimeValue.timeValueMillis(500),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_WARN_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.fetch.warn", TimeValue.timeValueMillis(10000),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_INFO_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.fetch.info", TimeValue.timeValueMillis(5000),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_DEBUG_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.fetch.debug", TimeValue.timeValueMillis(2000),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<TimeValue> INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_TRACE_SETTING =
        Setting.timeSetting(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".threshold.fetch.trace", TimeValue.timeValueMillis(500),
            TimeValue.timeValueMillis(-1), Property.Dynamic, Property.IndexScope);
    public static final Setting<SlowLogLevel> INDEX_SEARCH_SLOWLOG_LEVEL =
        new Setting<>(TIERED_STORAGE_SEARCH_SLOWLOG_PREFIX + ".level", SlowLogLevel.TRACE.name(), SlowLogLevel::parse, Property.Dynamic,
            Property.IndexScope);

    public static final Map<String, Setting<?>> TIERED_STORAGE_SEARCH_SLOWLOG_SETTINGS_MAP = Collections.unmodifiableMap(
        new HashMap<>() {
            {
                put(TIERED_STORAGE_SEARCH_SLOWLOG_ENABLED.getKey(),
                    TIERED_STORAGE_SEARCH_SLOWLOG_ENABLED);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_WARN_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_WARN_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_INFO_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_INFO_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_DEBUG_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_DEBUG_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_TRACE_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_QUERY_TRACE_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_WARN_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_WARN_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_INFO_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_INFO_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_DEBUG_SETTING.getKey(),
                    INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_DEBUG_SETTING);
                put(INDEX_SEARCH_SLOWLOG_THRESHOLD_FETCH_TRACE_SETTING.getKey(),
    ...
    public TieredStorageSearchSlowLog(IndexSettings indexSettings) {
        ...
        indexSettings.getScopedSettings().addSettingsUpdateConsumer(TIERED_STORAGE_SEARCH_SLOWLOG_ENABLED, this::setTieredStorageSlowlogEnabled);
    ...
    private void setMetricCollector(SearchContext searchContext, boolean isQueryPhase) {
        ...
        TieredStorageQueryMetricService.getInstance().addMetricCollector(Thread.currentThread().threadId(),
    ...
    public void onQueryPhase(SearchContext context, long tookInNanos) {
        ...
        final List<TieredStoragePerQueryMetric> metricCollectors = new ArrayList<>(removeMetricCollectors(
        ...
        if (tieredStorageSlowlogEnabled) {
            printSlowLog(context, tookInNanos, metricCollectors, queryWarnThreshold, queryLogger, queryInfoThreshold, queryDebugThreshold, queryTraceThreshold);
        }
    }
    ...
    private void printSlowLog(
        ...
        long warnThreshold, Logger logger, long infoThreshold, long debugThreshold, long traceThreshold) {
        if (warnThreshold >= 0 && tookInNanos > warnThreshold) {
            logger.warn("{}", new TieredStorageSlowLogPrinter(context, tookInNanos, metricCollectors));
        } else if (infoThreshold >= 0 && tookInNanos > infoThreshold) {
            logger.info("{}", new TieredStorageSlowLogPrinter(context, tookInNanos, metricCollectors));
        } else if (debugThreshold >= 0 && tookInNanos > debugThreshold) {
            logger.debug("{}", new TieredStorageSlowLogPrinter(context, tookInNanos, metricCollectors));
        } else if (traceThreshold >= 0 && tookInNanos > traceThreshold) {
            logger.trace("{}", new TieredStorageSlowLogPrinter(context, tookInNanos, metricCollectors));
        }
    }
    ...
    public void onFetchPhase(SearchContext context, long tookInNanos) {
        ...
        if (tieredStorageSlowlogEnabled) {
            ...
            // It's also much cleaner than propagating the fetch boolean to TIERED_STORAGEQueryMetricService
            final List<TieredStoragePerQueryMetric> metricCollectors = new ArrayList<>(removeMetricCollectors(
                context.getTask().getParentTaskId().toString(),
                context.shardTarget().getShardId().toString(),
                false
            ...
            printSlowLog(context, tookInNanos, metricCollectors, fetchWarnThreshold, fetchLogger, fetchInfoThreshold, fetchDebugThreshold, fetchTraceThreshold);
        }
    }
