...
public class OnDemandPrefetchBlockSnapshotIndexInput extends OnDemandBlockSnapshotIndexInput {
    ...
    protected IndexInput fetchBlock(int blockId) throws IOException {
        ...
        final TieredStoragePerQueryMetric metricCollector = TieredStorageQueryMetricService.getInstance().getMetricCollector(
            Thread.currentThread().threadId()
        );
        metricCollector.recordFileAccess(blockFileName, cacheHit);
        // fetch next N blocks asynchronously if read ahead is enabled
    ...
    protected boolean checkIfFileEnabledReadAhead() {
        return tieredStoragePrefetchSettingsSupplier.get().getReadAheadEnableFileFormats().stream()
